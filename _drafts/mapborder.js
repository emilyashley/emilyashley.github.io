...
function shrink(coord){
  if (!Array.isArray(coord[0][0])){
    return coord;
  }
  return shrink(_.flatten(coord, true));
 };
...
function geoList(coordinates){
  var uniqCoord = 
    _.uniq(
      _.map(shrink(coordinates), stringify)
        );
  return uniqCoord;
};
...
function overlap(list1, list2){
  return revert(_.intersection(list1, list2));
}


var canada = geoList(geojson.features[5].geometry.coordinates);
var usa = geoList(geojson.features[3].geometry.coordinates);
var mexico = geoList(geojson.features[20].geometry.coordinates);

var mexicoborder = overlap(usa, mexico);
var canadaborder = overlap(usa, canada);
